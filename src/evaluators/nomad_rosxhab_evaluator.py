...
class NomadHabitatROSEvaluator(HabitatSimEvaluator):
    r"""An Habitat evaluator for the Nomad environment.
    There is no Agent in this evaluator, for agents are spawned directly from `visualnav-transformer`.
    """
    ...
    def __init__(
        ...
        node_name: str = "nomad_rosxhab_evaluator_node",
        launch_file_path: str = "path_to_launch_file",
        env_node_name: str = "nomad_env_node",
        ...
        if enable_physics:
            ...
            env_node_args = shlex.split(
                f"python src/nodes/habitat_env_node.py --node-name {self.env_node_name} --task-config {config_paths} --enable-physics-sim --use-continuous-agent --sensor-pub-rate {sensor_pub_rate} --change-env-vel-subscriber /mobile_base/commands/velocity"
            )
        else:
